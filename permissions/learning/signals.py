...
def user_post_delete(sender, **kwargs):
    ...
    if user.username != settings.ANONYMOUS_USER_NAME:
        try:
            read_permissions_group = Group.objects.get(name=f"{user.username}: Read")
            read_permissions_group.delete()
        except Group.DoesNotExist:
            pass

        try:
            write_permissions_group = Group.objects.get(name=f"{user.username}: Write")    
            write_permissions_group.delete()
        except Group.DoesNotExist:
            pass

        try:
            own_permissions_group = Group.objects.get(name=f"{user.username}: Own")
